...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ error: error.message || 'Failed to send message via email provider.' });
    }

    return res.status(200).json({ success: true, data });
  } catch (err: any) {
    console.error('Error in /api/contact:', err);
    return res.status(500).json({ error: err.message || 'An unexpected error occurred while sending message.' });
  }
}
